...
LEDGER=$(post "/ledgers" <<<'{"name":"Cards"}' | jq -r .id)
...
balances() {
  get "/accounts/$CARD" | jq -c '{posted: .balances.posted.amount, available: .balances.available.amount}'
}
```
...
```sh
post "/transactions" "load-1-$LEDGER" <<EOF
...
```sh
FUEL=$(post "/holds" "auth-fuel-$LEDGER" <<EOF | jq -r .id
...
```sh
post "/holds/$FUEL/capture" "capture-fuel-$LEDGER" <<EOF | jq -c '{status, captured_amount}'
...
```sh
post "/settlements" "network-$LEDGER" <<EOF | jq -r .amount
